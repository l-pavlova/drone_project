...
def load_route():
    """Waypoints that follow the streets, generated by generate_world.py and
    written (in the same local metres as the world) to worlds/route.json. Fall
    back to a blind square lawnmower if that file is missing."""
    path = os.path.join(WORLDS, "route.json")
    try:
        ...
        print(f"[parkdrone] !! no usable route.json ({e}) -> lawnmower fallback")
    ...
    if settled:
        ...
        if idx < len(wps):
            ...
            if arrived:
                ...
                # heading PD: point the nose at the target (steer like a car; we
                # do NOT strafe with roll, which would tumble the drone).
