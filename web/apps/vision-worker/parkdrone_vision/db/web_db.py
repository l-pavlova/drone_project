...
def detail(conn, bay_id, history_limit=20):
    ...
    with conn.cursor() as cur:
        ...
        hcols = ["occupied", "frame_idx", "world", "votes_occupied", "views", "vis", "gt", "observed_at"]
...
def insert_mission(conn, mission_id, drone_id, world, area, frames_expected):
    with conn.cursor() as cur:
        cur.execute(
            """INSERT INTO mission (mission_id, drone_id, world, area, frames_expected)
               VALUES (%s, %s, %s, %s, %s)""",
            (mission_id, drone_id, world, area, frames_expected),
        )
...
def insert_frame(conn, frame_id, drone_id, mission_id, world, pose, image_uri):
    """Idempotent frame insert on (drone_id, world, i).
    ...
    with conn.cursor() as cur:
        cur.execute(
            """INSERT INTO frame
               ...
               RETURNING frame_id""",
            (
                frame_id, drone_id, mission_id, world, pose["i"],
        ...
        if row:
            return row[0], False
        cur.execute(
            "SELECT frame_id FROM frame WHERE drone_id = %s AND world = %s AND i = %s",
            (drone_id, world, pose["i"]),
        )
        return cur.fetchone()[0], True


def mark_frame_processed(conn, frame_id):
    """Mark a frame scored so startup crash-recovery won't re-enqueue it."""
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE frame SET status = 'processed', processed_at = now() WHERE frame_id = %s",
            (frame_id,),
        )
    conn.commit()
...
def mark_frame_failed(conn, frame_id):
    """Mark a frame unrecoverable (e.g. its image expired from the store) so
    crash-recovery stops re-enqueuing it every restart."""
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE frame SET status = 'failed', processed_at = now() WHERE frame_id = %s",
            (frame_id,),
        )
    conn.commit()
...
def unscored_frames(conn):
    """Frames still queued (never scored) — the crash-recovery backlog.

    Reconstructs the classify-job payload from the persisted frame row, so an
    in-memory queue lost on restart is rebuilt from Postgres + S3.
    """
    with conn.cursor() as cur:
        cur.execute(
            """SELECT frame_id, world, i, x, y, alt, yaw, roll, pitch, image_uri
                 FROM frame WHERE status = 'queued' ORDER BY received_at"""
        )
        rows = cur.fetchall()
    jobs = []
    for frame_id, world, i, x, y, alt, yaw, roll, pitch, image_uri in rows:
        jobs.append(
            {
                "frame_id": frame_id,
                "world": world,
